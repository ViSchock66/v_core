# SHAMASH — Context Manager / Memoria de Trabajo
> Inyeccion de contexto + historial de trabajo + lecciones aprendidas

## Identidad

- **Nombre:** SHAMASH
- **Rol:** Capa de persistencia — memoria semantica (ChromaDB), memoria de lecciones (SQLite `agent_memory`), historial de sesiones. **No es un agente con modelo LLM asignado** (corregido DT-20, 8 Jul 2026). El role `shamash` en `model_routing.yaml` que asigna `mistralai/mistral-small-4-119b-2603` es un remanente de una version anterior — SHAMASH no hace llamadas a la API NIM.
- **Embeddings:** importa de `api/embed.py` unificado (3-tier fallback: Ollama → NIM → hash SHA-256)
- **Vector DB:** ChromaDB en `chroma_db/` (collection `shamash_memory`)
- **Naturaleza:** solo lectura + escritura a SQLite/ChromaDB. No ejecuta archivos ni comandos.

## Responsabilidades

1. Inyectar contexto del proyecto ANTES de que ENKI/ENLIL actúen
...
- Ningun nodo del Task Graph que no sea "context" ejecuta sin que SHAMASH haya corrido primero
- SHAMASH solo lee — nunca escribe archivos ni ejecuta comandos
...
## Agentes que dependen de SHAMASH

| Agente | Que recibe de SHAMASH |
|---|---|
| ENLIL | Contexto del proyecto para clasificar y auditar |
| ENKI | Fragmentos de archivo con lineas de contexto para `str_replace` |
| NISABA | Mapa del proyecto y lecciones previas para busqueda semantica |